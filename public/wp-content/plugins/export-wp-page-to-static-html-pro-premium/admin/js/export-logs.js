  ...
  function export_logs_process(logs, r){
    if (logs !== "" || logs !== "[]") {
      ...
      if ($('#is_paused').val() == 'true'&& $('.log-paused').length){
        $('.log-paused').after(logHtml);
      }
      else{
  ...
  function generateHtmlExportLog(r, total_url_exported, total_urls_log) {
    if (r.creating_html_process == 'running') {
      ...
      $('.flat-button.pause, .flat-button.resume').hide();
    ...
    } else if (r.creating_html_process == 'paused') {
      rc_export_pages_paused();
  ...
  function logWorkers(r) {
    ...
    var total_urls_log = parseInt(r.total_urls_log);
    var total_url_exported = parseInt(r.total_url_exported);
  ...
  function export_log_create_html(log){
    ...
    var path = decodeURIComponent(log.path);
    //var path = decodeURIComponent(log.path);
  ...
  /*If export process paused this function will trigger*/
  function rc_export_pages_paused(text="") {
    $('.logs_list').prepend('<div class="log-paused"><span class="danger log_type">Paused</span> <span class="path"></span> <span class="comment">Please click on resume to continue.</span></div>');
